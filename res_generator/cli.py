...
from res_generator.spellcheck import spellcheck_resume
...
def main(config, input, outdir, company, debug_dump, debug_dir):
    ...
    # Spell Check
    issues = spellcheck_resume(data)
    if issues:
        click.echo("\n❗ Spell / grammar issues found:")
        for loc, word, msg in issues:
            click.echo(f"  • [{loc}] «{word}» — {msg}")
        if not click.confirm("Continue anyway?", default=False):
            _BrowserPool.close()
            raise SystemExit("Aborted due to language issues.")
